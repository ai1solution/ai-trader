...
class UniverseSelector:
    def __init__(self, config: Dict):
        ...
        # Criteria
        self.min_price = config.get('universe', {}).get('min_price', 0.01)
        self.min_volume = config.get('universe', {}).get('min_volume_24h', 1000000.0)
        self.min_atr_pct = config.get('universe', {}).get('min_atr_pct', 0.02) # 2% daily range
        ...
        for symbol in candidates:
            ...
            try:
                ...
                if refernece_time:
                    ...
                    df = await self.provider.fetch_ohlcv(symbol, '1d', limit=limit)
                ...
                # 2. Min Price
                if last_row['close'] < self.min_price:
                    print(f"[Universe] REJECT {symbol}: Price {last_row['close']} < {self.min_price}")
                    continue
